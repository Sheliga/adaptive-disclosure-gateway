...
REASON_CONFIRMATION_SECRET_NOT_DURABLE = "confirmation_secret_not_durable"
"""``GET /ready`` reason code (T25 review finding 2) for a service wired to a
provider outside the trust boundary whose ``PreviewConfirmationSigner`` is
only ephemeral (``is_durable`` is ``False``). Must match one of the values in
``application.wire.ReadinessReason``."""
...
class PreviewConfirmationSigner:
    ...
    def __init__(
        ...
        *,
        ...
        self._clock = clock
        # ``durable`` is metadata about how the secret was provisioned, never
        # the secret itself -- see ``is_durable``/``with_ephemeral_secret``.
    ...
    def with_ephemeral_secret(cls, **kwargs) -> PreviewConfirmationSigner:
        ...
        draws exactly that line and fails closed on the wrong side of it at
        startup; ``is_durable`` below lets ``GET /ready`` (T25 review
        finding 2) observe the same distinction at runtime.

        The same shape ``audit.py`` already uses for its per-process content
        hash key.
        """
        return cls(secret=secrets.token_bytes(MIN_SECRET_BYTES), durable=False, **kwargs)

    def __repr__(self) -> str:
        return (
            f"PreviewConfirmationSigner(ttl_seconds={self._ttl_seconds}, durable={self._durable})"
        )

    @property
    def is_durable(self) -> bool:
        """Whether this signer was built from configured, durable key
        material (``True``) rather than ephemeral per-process material
        generated by ``with_ephemeral_secret`` (``False``).

        Metadata only -- never exposes the secret itself, its length or any
        derived value. Read by
        ``DisclosureApplicationService.describe_readiness`` (``GET /ready``)
        to detect a deployment wired to an external provider whose
        confirmation signer would not survive a restart or be honoured by a
        second worker.
        """
