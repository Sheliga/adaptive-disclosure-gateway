...
The architecture foundation, deterministic detector, **B0 — Direct** (`DirectDiscloser`), **B1 — Static Sanitization** (`StaticSanitizer`), **B2 — Reversible Pseudonymization** (`ReversiblePseudonymizer`, with real REQUEST/DOCUMENT/SESSION pseudonym-scope lifecycles and offline-guessing-resistant opaque pseudonyms), the shared external-provider boundary (`providers.invoke_provider` and the deterministic `FakeProvider`), and the shared end-to-end execution path plus structural audit trail (`pipeline.run_disclosure_case` and `audit.AuditRecord`) are implemented on branch `feat/t16-pseudonym-scope-lifecycles`, currently under validation. Milestone 1's functional exit criteria are met on that branch.
