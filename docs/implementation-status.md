...
- **Execução** — `strategy` (the requested interface choice, which can be `"recommended"`, never itself a `b0`–`b4` code) and `treatment` (the `b0`–`b4` code actually executed), with a short explanation of the distinction and no conclusion drawn from either value;
