...
Full suite: 164 tests (157 + 6 new: one parametrized over B1/B2 for defect 1, its B0 regression pin, two provider-failure tests for defect 3, and one each for defects 2 and 4). Verified with a 15x full-suite loop (bare `pytest`), 0 flakes; `ruff check .` / `ruff format --check .` clean.
