...
Last updated: 2026-09-07 (T19 / Issue #26 — end-to-end B0/B1/B2 execution path and structural audit trail — implemented on branch `feat/t16-pseudonym-scope-lifecycles`, in validation, alongside T16 / Issue #23, T11 / Issue #12 and T18 / Issue #25. Milestone 1's functional exit criteria are now met on this branch; see "Milestone 1 exit criteria" below for what that means precisely and what remains explicitly out of scope. A subsequent egress-surface review on PR #27 found and fixed six further defects in this same T19 work — see "T19 / Issue #26 — egress-surface review fixes" immediately below the main T19 entry.)
...
### T19 / Issue #26 — egress-surface review fixes (implemented on `feat/t16-pseudonym-scope-lifecycles`, PR #27, pending review/merge)

The T19 work above already guarded the *payload*, but a later review applying the project's egress-surface rule ("the payload is clean" does not prove non-disclosure — everything that can cross the local boundary is in scope: payload, task/prompt, provider request, exception messages and their `__context__`, logs, telemetry, audit records, derived hashes/identifiers) found four further defects in `pipeline.py`/`audit.py`, all fixed on this branch with TDD (failing test confirmed for the expected reason before each fix):

1. **Sensitive data present only in `request.task` reached the provider untouched for B1/B2.** `pipeline.py` passed `request.task` straight into `ProviderRequest` without ever running detection over it — detection and every treatment's `sanitize()` only ever inspect `request.text`. Fixed by running the same `Detector` over `request.task` in `run_disclosure_case` and failing the whole request closed (`BLOCK_REQUEST`, provider never called) if it detects anything — chosen over silently sanitizing the task because a task carrying sensitive data is a governance violation (task/prompt scaffolding is meant to be held constant across treatments, not a second channel for disclosure-controlled content), and blocking is the fail-closed reading this codebase already applies to a malformed span or an unresolvable pseudonym scope. **B0 — Direct is deliberately exempt** — it is the unsafe control by definition and must keep sending everything raw, or every comparison would be biased in its favor. The exemption is a capability marker (`DirectDiscloser.unsafe_control_baseline = True`, checked via a new `runtime_checkable` `pipeline.UnsafeControlTreatment` protocol), never a branch on `Treatment.DIRECT` specifically — consistent with `tests/test_pipeline.py`'s existing structural pin against treatment-specific branching. Covered by `tests/test_pipeline.py::test_sensitive_value_present_only_in_task_causes_b1_and_b2_to_fail_closed_and_never_reach_the_provider` (parametrized over B1/B2) and a permanent regression pin, `tests/test_pipeline.py::test_b0_direct_still_sends_a_sensitive_task_to_the_provider_unchanged_as_the_unsafe_control`, so B0's exemption cannot be quietly "fixed" away later.
2. **The default audit produced a public, reproducible digest of sensitive content.** `audit.py::_content_hash` was a plain unkeyed `hashlib.sha256(value).hexdigest()`, applied to `external_payload`, `provider_response.text`, and `reconstructed_text` (which carries original values) — the same defect class as issue #24's pseudonym digest, letting anyone holding a default "metadata-only" audit record dictionary-attack those digests for low-entropy content (an 11-digit CPF, a name). Fixed with HMAC-SHA256 keyed by a per-process, CSPRNG-generated key (`secrets.token_bytes(32)`, generated once at module import, never derived from the content it hashes and never stored on `AuditRecord`, logged, or otherwise exposed) — the same reasoning already applied to `vault/in_memory.py`'s pseudonym tokens. The key is stable within a process/run (so hashes of equal content across audit records from the same run still compare equal, preserving the hash's actual purpose) but not reproducible across independent runs. `build_audit_record` takes an injectable `audit_hash_key` parameter so tests can get deterministic hashes while production always gets the non-reproducible default. Covered by `tests/test_audit.py::test_default_content_hash_is_keyed_and_does_not_match_an_offline_unkeyed_sha256`.
3. **A provider error or timeout produced no audit at all.** `invoke_provider` raises `ProviderError`/`ProviderTimeoutError`, and `run_disclosure_case` did not catch it, so the exception escaped the call entirely and the execution ended with no `AuditRecord` — fail-closed became unobservable exactly when observability mattered most. Fixed by catching `ProviderError` around the `invoke_provider` call in `run_disclosure_case`: on failure, `provider_response`/`reconstructed_text` stay `None` (no fallback to B0 — Direct, no retry — unchanged from `invoke_provider`'s own contract), and the audit record still gets built, now recording `ProviderStage.called=True, failed=True, failure_kind=<exception class name>` — never the exception's own message, since a third-party provider client can echo request content in it (the same reasoning `invoke_provider`'s existing `from None` chaining already applies one layer down). Covered by `tests/test_pipeline.py::test_provider_error_fails_closed_with_a_metadata_only_audit_record_instead_of_an_unhandled_exception` and its timeout sibling, both of which also assert the offending exception text and every raw fixture value are absent from the audit record's full JSON serialization.
4. **`decoding_config` was not recorded in the audit.** `ProviderStage` carried `provider_class`, `model_id`, `model_snapshot`, `transmitted_bytes` and `response_hash`, but not the decoding configuration issue #12 requires (model id, snapshot *and* decoding configuration) for a run to be reproducible. Fixed by adding `ProviderStage.decoding_config`, populated from `ProviderResponse.decoding_config` whenever a response was actually received. Covered by `tests/test_audit.py::test_audit_provider_stage_records_decoding_config_for_reproducibility`.
5. **A `provider_class` mismatch was audited as `called=True` even though `provider.generate` was never invoked.** `run_disclosure_case` set `provider_attempted = True` *before* calling `invoke_provider`, so `ProviderStage.called` (whose own docstring says "`called` is `True` whenever a provider was actually invoked") was `True` for `ProviderClassMismatchError` too — raised by `invoke_provider` (`providers/base.py`) *before* `executor.submit(provider.generate, request)`, specifically so the mismatched provider is never actually invoked. The audit contradicted its own documented contract for exactly the case it exists to make observable. Fixed at the source of truth rather than by special-casing the exception type in the pipeline (which would silently regress the moment a future pre-flight check is added to `invoke_provider` before the `generate` call): `ProviderError` now carries a `provider_invoked: bool` attribute, defaulting to `True` (every existing failure path except the provider_class check happens after `generate` is submitted); `ProviderClassMismatchError.__init__` unconditionally sets it `False` — there is no way to construct one with it `True`, since every raise site for that specific exception is, by construction, the pre-flight check. `run_disclosure_case` now reads `exc.provider_invoked` from the caught `ProviderError` instead of inferring `called` from a flag set before the call. `ProviderStage.provider_class` needed no separate change: it was already gated `provider_class if attempted else None` in `build_audit_record`, so correcting `attempted`/`called` for the mismatch case automatically also stops recording the mismatching provider's own class as though it had been used — deliberately left as this single derived gate rather than adding a second, independent condition that could drift out of sync with it. Covered by `tests/test_pipeline.py::test_provider_class_mismatch_never_calls_generate_and_audit_does_not_claim_it_was_called` (a recording stub proves `generate` is called zero times, and the full `AuditRecord` is serialized to JSON and checked against every raw HR fixture value) and three unit tests in `tests/test_providers.py` pinning `provider_invoked` in both directions: `False` for `ProviderClassMismatchError`, and `True` for a generic in-flight `ProviderError` and for `ProviderTimeoutError` (regression guard: the previously-fixed defect 3 behavior — `called=True, failed=True` for a real error/timeout — is unchanged).
6. **`ProviderError.provider_invoked` defaulted to `True`, an unfalsifiable-audit risk left latent by defect 5's own fix.** Defect 5 above added the `provider_invoked` attribute defaulting to `True`, justified by an assumption that was true only *today*: every failure site in `invoke_provider` except the provider_class pre-flight check happens after `executor.submit(provider.generate, request)`. That assumption rots — if a future pre-flight check is ever added to `invoke_provider` before the `generate` call and its `raise ProviderError(...)` forgets to pass `provider_invoked=False` explicitly (the same mistake defect 5 fixed for `ProviderClassMismatchError` specifically, but not for the general case), the audit record would silently *assert* a provider call that never happened — unfalsifiable from the record alone, since nothing about it looks wrong. Fixed by inverting the default to `False`: for an audit trail, the same class of mistake against a `False` default merely *omits* a call that did happen, an understatement that is at least cross-checkable against telemetry, whereas the `True` default made the mistake unfalsifiable. `invoke_provider` (`providers/base.py`) now sets `provider_invoked=True` explicitly at each of its three genuinely-post-submit failure sites (the timeout raise, the generic-exception-to-`ProviderError` translation, and — newly covered — the pass-through case where `provider.generate` itself raises a `ProviderError`/subclass directly, corrected on the way out rather than trusted from however the provider constructed it). `ProviderClassMismatchError`'s unconditional `provider_invoked=False` from defect 5 is unchanged and now agrees with, rather than overrides, the base default. TDD: the regression test constructs a bare `ProviderError` the way a future pre-flight check would (no explicit `provider_invoked`) and asserts `provider_invoked is False`, confirmed failing (`True is False`) before the default was inverted. Covered by `tests/test_providers.py::test_a_bare_provider_error_defaults_to_provider_invoked_false_so_a_future_preflight_check_cannot_silently_claim_a_call` (the new default-policy pin) and `tests/test_providers.py::test_provider_error_raised_directly_by_generate_is_corrected_to_provider_invoked_true` (the previously-uncovered pass-through branch); all three previously-existing `provider_invoked` behaviors (in-flight `ProviderError` → `True`, timeout → `True`, provider_class mismatch → `False`, `called=False`, `provider_class` null) reconfirmed unchanged.

Full suite: 170 tests (168 + 2 new: the default-policy regression test and the pass-through-branch test for defect 6). Verified with a 10x full-suite loop (bare `pytest`), 0 flakes; `ruff check .` / `ruff format --check .` clean.

Full T19 suite (pre-review-fixes baseline): 157 tests.
...
- CI is green — pending the CI run on this branch's PR; the full local suite (164 tests, after the egress-surface review fixes above) passes, including a 15-iteration full-suite repeat with zero flakes (see "Standing invariants" note carried in the PR description).
