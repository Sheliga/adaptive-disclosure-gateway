...
Full suite: 168 tests (164 + 4 new: one end-to-end pipeline test for defect 5, plus three `provider_invoked` unit tests in `tests/test_providers.py`). Verified with a 10x full-suite loop (bare `pytest`), 0 flakes; `ruff check .` / `ruff format --check .` clean.
